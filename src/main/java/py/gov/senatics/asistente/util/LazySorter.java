...
public class LazySorter<T> implements Comparator<T> {
	...
	public int compare(T obj1, T obj2) {
		...
		try {
			...
			String fieldName = this.sortField.substring(0, 1).toUpperCase()
					+ this.sortField.substring(1);
			...
			int value = CompareToBuilder.reflectionCompare(
					classType.cast(value1), classType.cast(value2));
